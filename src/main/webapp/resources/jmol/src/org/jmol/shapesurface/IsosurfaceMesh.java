...
public class IsosurfaceMesh extends Mesh {
  ...
  List[] getContours() {
    ...
    if (n < 0)
      n = -1 - n;
    List[] vContours = jvxlData.vContours;
    if (vContours != null) {
      for (int i = 0; i < n; i++) {
    ...
    //dumpData();
    vContours = new ArrayList[n];
    for (int i = 0; i < n; i++)
      vContours[i] = new ArrayList();
  ...
  public static void setContourVector(List v, int nPolygons,
  ...
  public static void addContourPoints(List v, BitSet bsContour, int i,
