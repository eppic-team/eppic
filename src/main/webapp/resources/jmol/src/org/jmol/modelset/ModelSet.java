...
*/  
  ...
  public void setPdbConectBonding(int baseAtomIndex, int baseModelIndex,
    ...
    for (int i = baseModelIndex; i < modelCount; i++) {
      List vConnect = (List) getModelAuxiliaryInfo(i, "PDB_CONECT_bonds");
      if (vConnect == null)
        continue;
        ...
        if (!models[bonds[i].atom1.modelIndex].isModelKit)
          if (bonds[i].isHydrogen()
            ...
            sb.append(bond.atom1.index)
                ...
                .append('\t').append(bond.mad / 1000f) // DARN! diameter, not radius... oh well...
   ...
   */
  public BitSet addHydrogens(List vConnections, Point3f[] pts) {
  ...
  public void connect(float[][] connections) {
    ...
    for (int i = 0; i < connections.length; i++) {
      ...
      // sad to say I used DIAMETER here, not radius....
