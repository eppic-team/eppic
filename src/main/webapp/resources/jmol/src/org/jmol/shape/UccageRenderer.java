/* $RCSfile$
 * $Author: hansonr $
 * $Date: 2010-10-29 10:52:10 -0500 (Fri, 29 Oct 2010) $
 * $Revision: 14547 $
 *
...
public class UccageRenderer extends CageRenderer {
  ...
  private void renderInfo(SymmetryInterface symmetry) {
    ...
    g3d.drawStringNoSlab("a="
        + nfformat(symmetry.getUnitCellAsArray(JmolConstants.INFO_A)) + "\u00C5",
        null, x, y, 0);
    if (!isPolymer) {
      y += lineheight;
      g3d.drawStringNoSlab(
          "b=" + nfformat(symmetry.getUnitCellAsArray(JmolConstants.INFO_B))
              + "\u00C5", null, x, y, 0);
    }
    if (!isPolymer && !isSlab) {
      y += lineheight;
      g3d.drawStringNoSlab(
          "c=" + nfformat(symmetry.getUnitCellAsArray(JmolConstants.INFO_C))
              + "\u00C5", null, x, y, 0);
    }
    if (nf != null)
    ...
    if (!isPolymer) {
      if (!isSlab) {
        ...
        g3d.drawStringNoSlab("\u03B1="
            + nfformat(symmetry.getUnitCellAsArray(JmolConstants.INFO_ALPHA))
            + "\u00B0", null, x, y, 0);
        y += lineheight;
        g3d.drawStringNoSlab("\u03B2="
            + nfformat(symmetry.getUnitCellAsArray(JmolConstants.INFO_BETA))
            + "\u00B0", null, x, y, 0);
      }
      y += lineheight;
      g3d.drawStringNoSlab("\u03B3="
          + nfformat(symmetry.getUnitCellAsArray(JmolConstants.INFO_GAMMA))
          + "\u00B0", null, x, y, 0);
    }
  }
