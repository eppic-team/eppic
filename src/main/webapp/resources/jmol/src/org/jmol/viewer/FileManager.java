  ...
  String getFileTypeName(String fileName) {
    ...
    try {
      ...
      if (br instanceof BufferedReader) {
        type = viewer.getModelAdapter().getFileTypeName((BufferedReader) br);
        ((BufferedReader) br).close();
      } else if (br instanceof InputStream) {
        String zipDirectory = getZipDirectoryAsString(fileName);
        type = (zipDirectory.indexOf("JmolManifest") >= 0 ? "Jmol" 
            : viewer.getModelAdapter().getFileTypeName(
              getBufferedReaderForString(zipDirectory)));
        ((InputStream) br).close();
  ...
  Object createAtomSeCollectionFromArrayData(List arrayData,
    ...
    for (int i = 0; i < nModels; i++) {
      ...
      if (data instanceof String)
        ...
        readers[i] = new VectorDataReader((List) arrayData.get(i));
    }
    FilesReader filesReader = new FilesReader(fullPathNames, fullPathNames,
  ...
  Object getUnzippedBufferedReaderOrErrorMessageFromName(
    ...
    try {
      ...
      if (CompoundDocument.isCompoundDocument(bis)) {
        ...
        if (asInputStream)
          return (BufferedInputStream) ZipUtil.getZipFileContents(bis, subFileList, 1,
              true);
...
//        else if (name.indexOf("/") == 0 && viewer.isSignedApplet())
   ...
   *          List of [filename1, bytes|null, filename2, bytes|null, ...]
   * @param preservePath
   * @param msg 
