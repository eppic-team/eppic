...
public abstract class BioPolymer extends Polymer {
  ...
  public int getPolymerPointsAndVectors(int last, BitSet bs, List vList,
  ...
  public Map getPolymerInfo(BitSet bs) {
    Map returnInfo = new Hashtable();
    List info = new ArrayList();
    List structureInfo = null;
    ProteinStructure ps;
    ProteinStructure psLast = null;
    int n = 0;
    for (int i = 0; i < monomerCount; i++) {
      if (bs.get(monomers[i].leadAtomIndex)) {
        Map monomerInfo = monomers[i].getMyInfo();
  ...
  public String calculateStructures(Polymer[] bioPolymers, int bioPolymerCount, List vHBonds, boolean doReport, boolean dsspIgnoreHydrogens, boolean setStructure) {
