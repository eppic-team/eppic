...
case 1076887572:
this.checkLength (2);
if (!this.chk) this.vwr.assignAromaticBonds ();
return;
case 1612189718:
if (this.e.slen != 2) {
asDSSP = (this.tokAt (++this.e.iToken) == 1641025539);
if (asDSSP) bs1 = this.vwr.getSelectedAtoms ();
...
bs1 = (this.e.iToken + 1 < this.slen ? this.atomExpressionAt (++this.e.iToken) : this.vwr.getSelectedAtoms ());
this.checkLength (++this.e.iToken);
...
var looping = !this.vwr.getAnimationReplayMode ().name ().equals ("ONCE");
var tok = this.tokAt (1);
var sfps = "";
switch (tok) {
case 0:
...
params.put ("captureMode", J.script.T.nameOf (mode).toLowerCase ());
...
this.checkLength (2);
}if (!this.chk) this.vwr.setCenterAt (tok, pt);
});
...
if (isFrames) {
var bsModels = this.vwr.getModelBitSet (bsFrom, false);
...
var ptsA = this.vwr.getAtomPointVector (bsFrom);
...
if (this.vwr.rotateAboutPointsInternal (this.e, center, pt1, endDegrees / nSeconds, endDegrees, doAnimate, bsFrom, translation, ptsB, null) && doAnimate && this.e.isJS) throw  new J.script.ScriptInterruption (this.e, "compare", 1);
}
});
...
var bsAtoms;
if (this.slen == 1) {
bsAtoms = this.vwr.setConformation ();
this.vwr.addStateScriptRet ("select", null, this.vwr.getSelectedAtoms (), null, "configuration", true, false);
} else {
var n = this.intParameter (this.e.checkLast (1));
if (this.chk) return;
bsAtoms = this.vwr.getConformation (this.vwr.getCurrentModelIndex (), n - 1, true);
...
var maps = null;
try {
maps = this.vwr.getSmilesMatcher ().getCorrelationMaps (smarts, atoms, ac, this.vwr.getSelectedAtoms (), true, false);
} catch (ex) {
if (Clazz_exceptionOf (ex, Exception)) {
...
JW.BSUtil.invertInPlace (bs2, this.vwr.getAtomCount ());
bs2.and (this.vwr.getAtomsWithinRadius (5, bs, false, null));
points.addLast (bs2);
}break;
case 10:
case 1048577:
...
var text = null;
if (font != null) text = (J.api.Interface.getInterface ("JM.Text")).newLabel (this.vwr.getGraphicsData (), font, "", colix, 0, 0, 0, null);
...
atomSets[0] = atomSets[1] = this.vwr.getSelectedAtoms ();
var radius = NaN;
var colorArgb = [-2147483648];
var distanceCount = 0;
...
if (!this.chk) this.vwr.setPdbConectBonding (isAuto, eval.$isStateScript);
return;
case 1073741830:
case 1073741852:
case 1073741904:
...
break;
case 1073741882:
if (!this.chk) this.vwr.clearConsole ();
break;
case 135270422:
...
if (!this.chk) eval.runScript (this.vwr.getSymmetryInfo (bsAtoms, xyz, iSym, center, target, thisId, 135176));
return false;
case 4115:
isFrame = true;
continue;
...
this.vwr.setLigandModel (dataLabel.substring (5).toUpperCase () + "_file", dataString.trim ());
return;
...
var bs = this.vwr.getSelectedAtoms ();
d[0] = dataType;
var atomNumberField = (isOneValue ? 0 : (this.vwr.getParameter ("propertyAtomNumberField")).intValue ());
var atomNumberFieldColumnCount = (isOneValue ? 0 : (this.vwr.getParameter ("propertyAtomNumberColumnCount")).intValue ());
var propertyField = (isOneValue ? -2147483648 : (this.vwr.getParameter ("propertyDataField")).intValue ());
var propertyFieldColumnCount = (isOneValue ? 0 : (this.vwr.getParameter ("propertyDataColumnCount")).intValue ());
...
break;
case 1089470478:
if (modelIndex < 0) modelIndex = Math.min (this.vwr.getCurrentModelIndex (), 0);
var needIgnore = (bsIgnore == null);
if (bsSelect == null) bsSelect = JW.BSUtil.copy (this.vwr.getSelectedAtoms ());
bsSelect.and (this.vwr.getAtomBits (1297090050, Integer.$valueOf (1)));
if (!needIgnore) bsSelect.andNot (bsIgnore);
this.addShapeProperty (propertyList, "select", bsSelect);
if (needIgnore) {
bsIgnore = JW.BSUtil.copy (bsSelect);
JW.BSUtil.invertInPlace (bsIgnore, this.vwr.getAtomCount ());
isFrontOnly = true;
...
pts = this.vwr.getBoundBoxVertices ();
...
bs.and (this.vwr.getAtomsWithinRadius (5.0, bsSelect, false, null));
} else {
bs = this.vwr.getAtomsWithinRadius (5.0, bsSelect, true, null);
bs.andNot (this.vwr.getAtomBits (1095761936, bsSelect));
}bs.andNot (bsSelect);
...
}if (ptc == null) ptc = (bs == null ?  new JU.P3 () : this.vwr.getAtomSetCenter (bs));
...
if (tokProperty != 1766856708) for (var iAtom = ac; --iAtom >= 0; ) data[iAtom] = JM.Atom.atomPropertyFloat (this.vwr, atoms[iAtom], tokProperty);
...
JW.Logger.info (sfdat);
sfdat = JU.PT.split (sfdat, "MAP_SIGMA_DENS")[1];
cutoff = JU.PT.parseFloat (sfdat);
JW.Logger.info ("using cutoff = " + cutoff);
} catch (e) {
if (Clazz_exceptionOf (e, Exception)) {
JW.Logger.error ("MAP_SIGMA_DENS -- could  not read " + info[1]);
} else {
throw e;
}
}
...
onlyOneModel = "=xxxx";
if (modelIndex < 0) modelIndex = this.vwr.getCurrentModelIndex ();
...
}if (firstPass && this.vwr.getParameter ("_fileType").equals ("Pdb") && Float.isNaN (sigma) && Float.isNaN (cutoff)) {
...
var ce = this.vwr.getColorEncoder (colorScheme);
if (ce != null) {
...
if (needSelect) bsSelect = JW.BSUtil.copy (this.vwr.getSelectedAtoms ());
if (modelIndex < 0) modelIndex = this.vwr.getCurrentModelIndex ();
bsSelect.and (this.vwr.getModelUndeletedAtomsBitSet (modelIndex));
if (onlyOneModel != null) {
var bsModels = this.vwr.getModelBitSet (bsSelect, false);
...
if (Clazz_instanceOf (area, Float)) this.vwr.setFloatProperty ("isosurfaceArea", (area).floatValue ());
 else this.vwr.setUserVariable ("isosurfaceArea", J.script.SV.getVariableAD (area));
}if (doCalcVolume) {
...
if (Clazz_instanceOf (volume, Float)) this.vwr.setFloatProperty ("isosurfaceVolume", (volume).floatValue ());
 else this.vwr.setUserVariable ("isosurfaceVolume", J.script.SV.getVariableAD (volume));
}if (!isLcaoCartoon) {
var s = null;
...
sarea = (doCalcArea ? "isosurfaceArea = " + (Clazz_instanceOf (area, Float) ? "" + area : JW.Escape.eAD (area)) : null);
svol = (doCalcVolume ? "isosurfaceVolume = " + (Clazz_instanceOf (volume, Float) ? "" + volume : JW.Escape.eAD (volume)) : null);
if (s == null) {
...
bsFrom = this.vwr.getSelectedAtoms ();
...
var bsSubset = JW.BSUtil.copy (this.vwr.getSelectionSubset ());
...
eval.setBooleanProperty ("navigationMode", true);
this.vwr.setNavOn (eval.theTok == 1048589);
return;
case 1073742162:
if (!this.chk) this.vwr.setNavXYZ (0, 0, 0);
return;
case 8:
case 1113200654:
break;
...
if (!this.chk) this.vwr.setNavXYZ (pt.x, pt.y, pt.z);
return;
case 554176526:
...
if (!this.chk) this.vwr.navigateList (eval, list);
});
...
modelIndex = this.vwr.getJmolDataSourceFrame (modelIndex);
...
var propertyY = 0;
var propertyZ = 0;
var bs = JW.BSUtil.copy (this.vwr.getSelectedAtoms ());
...
var ptDataFrame = this.vwr.getJmolDataFrameIndex (modelIndex, stateScript);
...
var data = (type.equals ("data") ? "1 0 H 0 0 0 # Jmol PDB-encoded data" : this.vwr.getPdbData (modelIndex, type, parameters));
...
var color = (JW.C.getHexCode (this.vwr.getColixBackgroundContrast ()));
...
ss.setModelIndex (this.vwr.getCurrentModelIndex ());
...
var se =  new J.script.ScriptEval ().setViewer (this.vwr);
se.historyDisabled = true;
se.compiler =  new J.script.ScriptCompiler (this.vwr);
se.sm = shapeManager;
try {
...
}if (!this.chk) bsFrames = this.vwr.getModelBitSet (bsAtoms, true);
break;
case 1073742139:
...
if ("?".equals (fileName)) fileName = "?Jmol." + this.vwr.getParameter ("_fileType");
...
if (tVar == null) {
tVar = this.e.getParameter (J.script.SV.sValue (this.tokenAt (isCommand ? 2 : 1, args)), 1073742190);
}var v = null;
if (tVar.tok == 15) {
v =  new JU.List ();
...
break;
case 135270423:
if (!this.chk) msg = JW.Escape.e (this.vwr.cacheList ());
break;
case 1073741915:
this.checkLength (2);
...
if (!this.chk) msg = this.vwr.getSymmetryOperation (null, 0, pt1, pt2, false);
len = ++this.e.iToken;
} else {
var iop = (this.e.checkLength23 () == 2 ? 0 : this.intParameter (2));
if (!this.chk) msg = this.vwr.getSymmetryOperation (null, iop, null, null, false);
len = -3;
}break;
case 1649412120:
var vdwType = null;
...
break;
case 1073742192:
if (!this.chk) msg = this.vwr.getAtomDefs (this.e.definedAtomSets) + this.vwr.getVariableList () + this.getContext (true);
break;
case 536870926:
...
break;
case 536870918:
value = JW.Escape.eP (this.vwr.getDefaultLattice ());
break;
case 4126:
if (!this.chk) msg = this.vwr.getMinimizationInfo ();
break;
case 1611272194:
switch (this.vwr.getAxesMode ()) {
...
break;
case 1073741828:
msg = JW.Escape.eAP (this.vwr.getAdditionalHydrogens (null, true, true, null));
break;
case 1613758476:
...
case 1073742136:
case 4146:
if (!this.chk) msg = this.vwr.listSavedStates ();
break;
case 1614417948:
...
case 1048581:
if ((len = this.slen) == 2) {
if (!this.chk) msg = this.vwr.getCoordinateState (this.vwr.getSelectedAtoms ());
break;
}var nameC = this.paramAsStr (2);
if (!this.chk) msg = this.vwr.getSavedCoordinates (nameC);
break;
case 1073742158:
if (!this.chk) this.vwr.clearConsole ();
if ((len = this.slen) == 2) {
...
if (!this.chk) msg = this.vwr.getEmbeddedFileState (this.paramAsStr (3));
break;
}len = 3;
if (!this.chk) msg = this.vwr.getSavedState (name);
break;
case 1641025539:
if ((len = this.slen) == 2) {
...
if (!this.chk) msg = this.vwr.getSavedStructure (shape);
break;
case 135270408:
...
case 1679429641:
if (!this.chk) {
msg = this.vwr.getBoundBoxCommand (true);
}break;
case 12289:
if (!this.chk) msg = "center " + JW.Escape.eP (this.vwr.getRotationCenter ());
break;
case 135176:
...
case 1095766030:
if (!this.chk) msg = this.vwr.getModelInfoAsString ();
break;
case 537006096:
...
return;
case 1089470478:
if (!this.chk) msg = this.vwr.getSymmetryInfoAsString ();
break;
case 1073742176:
if (!this.chk) msg = "transform:\n" + this.vwr.getTransformText ();
break;
case 4168:
msg = "zoom " + (this.vwr.getZoomEnabled () ? ("" + this.vwr.getZoomSetting ()) : "off");
break;
case 1611272202:
...
modelIndex = this.vwr.getAtomModelIndex (modelSet.bo[bondIndex].getAtomIndex1 ());
...
var modelIndex = modelSet.at[index].mi;
...
sb.append (J.script.ScriptError.getErrorLineMessage (context.functionName, context.scriptFileName, this.e.getLinenumber (context), context.pc, J.script.ScriptEval.statementAsString (this.vwr, context.statement, -9999, this.e.debugHigh)));
...
case 1679429641:
eval.iToken = i + 1;
data = JW.BoxInfo.getCriticalPoints (this.vwr.getBoundBoxVertices (), null);
break;
case 1073741872:
case 1614417948:
eval.iToken = i + 1;
...
if (this.chk) return;
if (modelIndex < 0) {
modelIndex = this.vwr.getCurrentModelIndex ();
...
var bbox = this.vwr.getBoxInfo (bs, -Math.abs (distance));
...
if (indices == null && label != null && label.indexOf ("%D") > 0) indices = this.vwr.getAtomIndices (bs);
...
if (asIdentity) str = modelSet.at[j].getInfo ();
 else str = labeler.formatLabelAtomArray (this.vwr, modelSet.at[j], tokens, '\0', indices);
} else {
var bond = modelSet.getBondAt (j);
if (asIdentity) str = bond.getIdentity ();
 else str = labeler.formatLabelBond (this.vwr, bond, tokens, htValues, indices);
