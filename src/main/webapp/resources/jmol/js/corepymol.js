...
Clazz_load (["java.util.Hashtable", "JU.List"], "J.adapter.readers.pymol.PickleReader", ["java.lang.Double", "$.Long", "JU.SB", "JW.Logger"], function () {
...
case 115:
o = this.pop ();
if (!(Clazz_instanceOf (this.peek (), String))) JW.Logger.error (this.peek () + " is not a string");
s = this.pop ();
(this.peek ()).put (s, o);
break;
case 117:
...
Clazz_declarePackage ("J.adapter.readers.pymol");
Clazz_load (["java.util.Hashtable"], "J.adapter.readers.pymol.PyMOL", ["JW.Logger"], function () {
...
Clazz_declarePackage ("J.adapter.readers.pymol");
Clazz_load (null, "J.adapter.readers.pymol.JmolObject", ["java.lang.Float", "JU.P3", "$.PT", "$.SB", "J.adapter.readers.pymol.PyMOLScene", "JW.BSUtil", "$.Escape"], function () {
c$ = Clazz_decorateAsClass (function () {
this.id = 0;
this.bsAtoms = null;
this.info = null;
...
JW.BSUtil.invertInPlace (bs, sm.vwr.getAtomCount ());
...
case 1073742139:
sm.vwr.saveScene (this.jmolName, this.info);
sm.vwr.saveOrientation (this.jmolName, (this.info).get ("pymolView"));
return;
case 5:
...
case 16:
sm.loadShape (this.id);
sm.setShapePropertyBs (this.id, "ignore", JW.BSUtil.copyInvert (this.bsAtoms, sm.vwr.getAtomCount ()), null);
break;
default:
...
if (this.modelIndex < 0) this.modelIndex = sm.vwr.getCurrentModelIndex ();
if (this.bsAtoms == null) {
...
break;
case 1073742018:
this.modelIndex = sm.vwr.getCurrentModelIndex ();
var mesh = this.info;
sID = mesh.get (mesh.size () - 2).toString ();
sb =  new JU.SB ();
sb.append ("isosurface ID ").append (JU.PT.esc (sID)).append (" model ").append (m.am[this.modelIndex].getModelNumberDotted ()).append (" color ").append (JW.Escape.escapeColor (this.argb)).append ("  ").append (JU.PT.esc (this.cacheID)).append (" ").append (JU.PT.esc (sID)).append (" mesh nofill frontonly");
...
Clazz_declarePackage ("J.adapter.readers.pymol");
Clazz_load (["J.api.JmolSceneGenerator", "java.util.Hashtable", "JU.BS", "$.List", "$.P3"], "J.adapter.readers.pymol.PyMOLScene", ["java.lang.Boolean", "$.Character", "$.Double", "$.Float", "JU.AU", "$.CU", "$.SB", "J.adapter.readers.pymol.JmolObject", "$.PyMOL", "$.PyMOLGroup", "J.atomdata.RadiusData", "J.c.VDW", "JM.Text", "JW.BSUtil", "$.C", "$.Escape", "$.Logger", "$.Point3fi"], function () {
c$ = Clazz_decorateAsClass (function () {
this.vwr = null;
this.pymolVersion = 0;
this.bsHidden = null;
...
if (ac >= 0) {
this.bsAtoms = JW.BSUtil.newBitSet2 (ac0, ac);
var jo;
...
var t = JM.Text.newLabel (this.vwr.getGraphicsData (), font, label, this.getColix (colorIndex, 0), 0, 0, 0, labelOffset);
return t;
}, "~S,~A,~N,~N,~N");
...
Clazz_declarePackage ("J.adapter.readers.pymol");
Clazz_load (["J.adapter.readers.pdb.PdbReader", "J.api.PymolAtomReader", "JU.BS", "$.P3"], "J.adapter.readers.pymol.PyMOLReader", ["java.lang.Boolean", "java.util.Hashtable", "JU.List", "$.PT", "$.V3", "J.adapter.readers.pymol.PickleReader", "$.PyMOL", "$.PyMOLScene", "J.adapter.smarter.Atom", "$.Bond", "$.Structure", "J.c.STR", "JW.BSUtil", "$.BoxInfo", "$.Logger"], function () {
c$ = Clazz_decorateAsClass (function () {
...
this.asc.setAtomSetCollectionAuxiliaryInfo ("noAutoBond", Boolean.TRUE);
this.asc.setAtomSetAuxiliaryInfo ("pdbNoHydrogens", Boolean.TRUE);
this.asc.setAtomSetCollectionAuxiliaryInfo ("isPyMOL", Boolean.TRUE);
...
}this.vwr.setTrajectoryBs (JW.BSUtil.newBitSet2 (this.baseModelIndex, this.vwr.ms.mc));
...
this.asc.setAtomSetCollectionAuxiliaryInfo ("perferredWidthHeight", [width, height]);
...
this.pymolScene.currentAtomSetIndex = this.asc.currentAtomSetIndex;
...
}if (this.$ac == 0) this.asc.setAtomSetCollectionAuxiliaryInfo ("dataOnly", Boolean.TRUE);
...
this.ssMapSeq = null;
JW.Logger.info ("reading " + (this.$ac - this.ac0) + " atoms");
JW.Logger.info ("----------");
return bsAtoms;
}, "JU.List,~N");
