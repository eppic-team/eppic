...
Clazz.load (["java.lang.Float", "JU.BS", "$.V3"], "JM.AtomCollection", ["java.lang.Character", "java.util.Arrays", "$.Hashtable", "JU.A4", "$.AU", "$.List", "$.M3", "$.P3", "$.PT", "J.api.Interface", "J.atomdata.RadiusData", "J.c.PAL", "$.STR", "$.VDW", "JM.Group", "J.script.T", "JW.BSUtil", "$.Elements", "$.Escape", "$.Logger", "$.Measure", "$.Parser", "$.Txt", "$.Vibration", "JV.JC"], function () {
...
function (i, format) {
if (format == null) return this.at[i].getInfo ();
return this.getLabeler ().formatLabel (this.vwr, this.at[i], format);
}, "~N,~S");
...
return this.at[i].getInfoXYZ (useChimeFormat);
}, "~N,~B");
...
function (i) {
return JW.Elements.elementNameFromNumber (this.at[i].getAtomicAndIsotopeNumber ());
}, "~N");
...
return (iAtom < 0 ? "null" : this.at[iAtom].group.getHelixData (tokType, this.vwr.getQuaternionFrame (), this.vwr.getInt (553648146)));
}, "JU.BS,~N");
...
function (atom) {
return JW.Elements.getVanderwaalsMar (atom.getElementNumber (), J.c.VDW.JMOL) / 1000;
}, "JM.Atom");
...
this.calcBfactorRange (this.vwr.getSelectedAtoms ());
} else {
...
ec.calculate ( new J.atomdata.RadiusData (null, envelopeRadius, J.atomdata.RadiusData.EnumType.ABSOLUTE, null), 3.4028235E38, bsSelected, JW.BSUtil.copyInvert (bsSelected, this.ac), false, false, false, true);
...
case 1826248716:
case 1288701959:
this.vwr.setAtomLabel (sValue, i);
break;
case 1129318401:
...
break;
case 1114638363:
this.vwr.setSelectedAtom (atom.i, (fValue != 0));
break;
case 1112541199:
...
if (tok == 1114638363) this.vwr.setSelectedAtom (-1, false);
}, "JU.BS,~N,~N,~N,~S,~A,~A");
...
this.occupancies =  Clazz.newByteArray (this.at.length, 0);
...
for (var i = 0; i < this.at.length; i++) this.hydrophobicities[i] = JW.Elements.getHydrophobicity (this.at[i].getGroupID ());
...
for (i = this.ac; --i >= 0; ) if (this.isAtomNameMatch (this.at[i], atomSpec, false)) bs.set (i);
...
for (var i = this.ac; --i >= 0; ) {
if (this.at[i].getElementNumber () != 8) continue;
var a = this.at[i];
var b;
var g = a.getGroupID ();
if (g >= 42 && g < 45) {
...
i++;
}} else if (this.isAtomNameMatch (this.at[i], name, checkStar)) {
if (bs == null) bs = JU.BS.newN (i + 1);
bs.set (i);
}}
return bs;
