...
Clazz.load (["javajs.api.GenericLineReader", "J.adapter.smarter.AtomSetCollectionReader", "JU.CifDataParser", "$.List", "$.P3"], "J.adapter.readers.cif.CifReader", ["java.lang.Boolean", "$.Character", "$.Float", "java.util.Hashtable", "JU.BS", "$.PT", "$.Rdr", "$.V3", "J.adapter.smarter.Atom", "J.api.Interface", "$.JmolAdapter", "JW.Logger"], function () {
...
}, J.adapter.readers.cif, "CifReader", J.adapter.smarter.AtomSetCollectionReader, javajs.api.GenericLineReader);
...
if (this.nAtoms == this.asc.ac) this.asc.removeCurrentAtomSet ();
...
if (header.length > 0) this.asc.setAtomSetCollectionAuxiliaryInfo ("fileHeader", header);
...
if (!this.data.equals ("?")) this.asc.setAtomSetCollectionAuxiliaryInfo ("modelLoadNote", this.data);
