...
Clazz.load (["J.api.JmolDataManager", "java.util.Hashtable"], "JV.DataManager", ["java.lang.Boolean", "JU.AU", "$.BS", "$.PT", "$.SB", "J.c.VDW", "J.script.T", "JW.BSUtil", "$.Elements", "$.Escape", "$.Logger", "$.Parser"], function () {
...
JW.Parser.parseFloatArrayFromMatchAndField (stringData, this.vwr.bsUserVdws, 1, 0, data[2], 2, 0, this.vwr.userVdws, 1);
...
for (var i = i0; i < i1 && i >= 0; i = (isAll ? i + 1 : bs.nextSetBit (i + 1))) sb.appendI (i).appendC ('\t').appendF (type === J.c.VDW.USER ? this.vwr.userVdws[i] : JW.Elements.getVanderwaalsMar (i, type) / 1000).appendC ('\t').append (JW.Elements.elementSymbolFromNumber (i)).appendC ('\n');
