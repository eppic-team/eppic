...
Clazz.load (["JU.A4", "$.M3", "$.M4", "$.P3", "$.P3i", "$.V3", "J.c.STER", "JW.Point3fi"], "JV.TransformManager", ["java.lang.Boolean", "$.Float", "java.util.Hashtable", "JU.P4", "$.Quat", "$.SB", "J.api.Interface", "JW.Escape", "$.Logger"], function () {
...
function (vwr, width, height) {
this.setViewer (vwr, width, height);
}, "JV.Viewer,~N,~N");
...
var m = this.vwr.getModelSetAuxiliaryInfoValue ("defaultOrientationMatrix");
...
if (this.vwr.g.axesOrientationRasmol) this.rotateX (3.141592653589793);
}this.vwr.saveOrientation ("default", null);
...
this.vwr.unTransformPoint (ptCamera, ptCamera);
...
this.zmPct = newZoom;
if (!this.vwr.g.fontCaching) this.vwr.getGraphicsData ().clearFontCache ();
...
}this.vwr.g.setS ("_slabPlane", JW.Escape.eP4 (this.getSlabDepthPlane (false)));
this.vwr.g.setS ("_depthPlane", JW.Escape.eP4 (this.getSlabDepthPlane (true)));
...
function (v, pt) {
v.setTempPoint (pt, this.vibrationT, this.vibrationScale, this.vwr.g.modulationScale);
return pt;
...
if (period > 0) return;
period = -period;
}this.setVibrationOn (period > 0 && this.vwr.modelGetLastVibrationIndex (this.vwr.getCurrentModelIndex (), 0) >= 0);
}, "~N");
...
switch (relativeTo) {
case 96:
pt1.add (this.vwr.getAverageAtomPoint ());
break;
case 1679429641:
...
Clazz.defineMethod (c$, "getTempScreenPt", 
