...
abstract public class MOReader extends BasisFunctionReader {
 ...
 -------------------------------------------------------------------------------
  ...
  protected int setMOType(Hashtable mo, int i) {
    if (moTypes != null) {
      String s = (String) moTypes.get(i % moTypes.size());
      i++;
      mo.put("type", s);
  ...
  protected void addMOData(int nColumns, List[] data, Hashtable[] mos) {
