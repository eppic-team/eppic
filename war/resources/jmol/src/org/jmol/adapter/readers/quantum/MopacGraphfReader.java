...
public class MopacGraphfReader extends MopacSlaterReader {
  ...
  private List orbitalData;
  private List orbitalInfo;
  ...
  private void readMolecularOrbitals(boolean isBeta) throws Exception {
    ...
    if (isNewFormat) {
      orbitalData = new ArrayList();
      if (line.length() > 10)
        orbitalInfo = new ArrayList();
    } else {
    ...
    if (orbitalData != null) {
      ...
      for (int i = nOrbitals; --i >= 0;)
        list[i] = (float[]) orbitalData.get(i);
    }
    ...
    for (int iMo = 0; iMo < nOrbitals; iMo++) {
      ...
      if (orbitalInfo != null) {
        line = (String) orbitalInfo.get(iMo);
