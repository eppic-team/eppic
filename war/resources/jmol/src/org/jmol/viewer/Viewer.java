/* $RCSfile$
 * $Author: hansonr $
 * $Date: 2011-01-26 13:51:05 -0600 (Wed, 26 Jan 2011) $
 * $Revision: 15043 $
 *
 ...
 * JmolAdapter. JmolViewer uses this interface to fextract information from the
...
*/
  ...
  private final static int FILE_STATUS_CREATING_MODELSET = 2; // not sent
  /**
  ...
  public String loadModelFromFile(String fullPathName, String fileName,
    ...
    if (fileNames != null) {
      ...
      for (int i = 0; i < fileNames.length; i++)
        s = TextFormat.simpleReplace(s, "$FILENAME" + (i + 1) + "$", Escape.escape(fileNames[i].replace('\\','/')));
      ...
      if (reader == null)
        ...
        atomSetCollection = fileManager.createAtomSetCollectionFromReader(
            fullPathName, fileName, (Reader) reader, htParams = setLoadParameters(null, isAppend));
      else

        // 3) a DOM reader (could be used by Jmol) 

        atomSetCollection = fileManager.createAtomSetCollectionFromDOM(
            reader, htParams = setLoadParameters(null, isAppend));
    }
...
//      fileManager.setFileInfo(saveInfo);
  ...
  public void zap(boolean notify, boolean resetUndo, boolean zapModelKit) {
    ...
    if (modelSet != null) {
      //setBooleanProperty("appendNew", true); should not be reset upon ZAP
      clearModelDependentObjects();
      fileManager.clear();
  ...
  public float getUnitCellInfo(int infoType) {
    ...
    if (symmetry == null)
      return Float.NaN;
    return symmetry.getUnitCellAsArray(infoType);
  }
  ...
  public String getStateInfo(String type) {
    ...
    //if (isAll)
      //s.append(getFunctionCalls(null, true)); removed in 12.0.19 because it is not necessary for the state
    // file state
    ...
    if (isAll || type.equalsIgnoreCase("variableState"))
      s.append(global.getState(sfunc)); // NO!!!! removed in 12.0.19 because it is not necessary for the state
  ...
  public void setScreenDimension(Dimension dim) {
    ...
    //System.out.println("Viewer.setScreenDimensions " + dim);
  ...
  synchronized Object evalStringWaitStatus(String returnType, String strScript,
    ...
    // get List of Vectors of Vectors info
  ...
  public BitSet addHydrogensInline(BitSet bsAtoms, List vConnections,
  ...
  public void setCursor(int cursor) {
    if (multiTouch || currentCursor == cursor || !haveDisplay)
      return;
    int c;
   ...
   * {}
   * 
   * FILE_STATUS_NOT_LOADED = -1 
   * FILE_STATUS_ZAPPED = 0
   * FILE_STATUS_CREATING_MODELSET = 2 
   * FILE_STATUS_MODELSET_CREATED = 3 
   * FILE_STATUS_MODELS_DELETED = 5
   * 
   * FILE_STATUS_CREATING_MODELSET is not passed on to the callback function. 
   * variable _loadPoint can be checked as well.
   * 
  ...
  private void jmolTest() {
    /*
     * List v = new ArrayList(); List m = new ArrayList(); v.add(m);
     * m.add("MODEL     2");m.add(
  ...
  public void rotateAboutPointsInternal(Point3f point1, Point3f point2,
                                        ...
                                        BitSet bsSelected,
                                        Vector3f translation, List finalPoints) {
  ...
  public Point3f[][] getCenterAndPoints(List atomSets, boolean addCenter) {
  ...
  private final List actionStates = new ArrayList();
  private final List actionStatesRedo = new ArrayList();
