...
////////////////////Jmol status //////////////
  ...
  synchronized private void setStatusChanged(String statusName,
    ...
    if (!recordStatus(statusName))
      return;
    List msgRecord = new ArrayList();
    msgRecord.add(new Integer(++statusPtr));
    msgRecord.add(statusName);
    msgRecord.add(new Integer(intInfo));
    msgRecord.add(statusInfo);
    List statusRecordSet = (isReplace ? null : (List)messageQueue.get(statusName));
    if (statusRecordSet == null)
      messageQueue.put(statusName, statusRecordSet = new ArrayList());
    if (statusRecordSet.size() == MAXIMUM_QUEUE_LENGTH)
      statusRecordSet.remove(0);    
    statusRecordSet.add(msgRecord);
  }
  
  synchronized List getStatusChanged(String newStatusList) {
    /*
     * returns a List of statusRecordSets, one per status type,
    ...
    if (getList) {
      ...
      while (e.hasMoreElements())
        msgList.add(messageQueue.remove(e.nextElement()));
    } else {
      messageQueue.clear();
    }
    statusPtr = 0;
