...
public class FullAnalysis {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(FullAnalysis.class);
    ...
    public FullAnalysis(EppicParams params, String pdbCode, boolean isInputAFile, File inFile) throws EppicException {
        this.params = params;
        this.pdbCode = pdbCode;
        this.isInputAFile = isInputAFile;
        this.inFile = inFile;
        this.name = inFile==null?pdbCode:inFile.getName();
    }

    public void run() throws EppicException {
    ...
    public long doLoadPdb() throws EppicException {
        ...
        System.out.println("Loading PDB data: "+name);
        ...
        try {
            if (!isInputAFile) {
                ...
                try {
                    pdb = readStructureFromPdbCode(fileParsingParams, pdbCode);
                    if (params.getCifRepositoryTemplateUrl()!=null) {
                        modDate = getLastModDateFromUrl(pdbCode.toLowerCase());
                    }

                } catch(IOException e) {
                    throw new EppicException(e,"Couldn't get cif file from AtomCache for code "+pdbCode+". Error: "+e.getMessage(),true);
                }

            } else {

                int fileType = FileTypeGuesser.guessFileType(inFile);
                fileParsingParams.setParseBioAssembly(false);

                if (fileType==FileTypeGuesser.CIF_FILE) {

                    pdb = CifStructureConverter.fromInputStream(IOUtils.openFile(inFile), fileParsingParams);
                    ...
                    pdb = parser.parsePDBFile(new FileInputStream(inFile));
            ...
            throw new EppicException(e,"Problems reading PDB data from "+inFile+". Error: "+e.getMessage(),true);
        }

        if (pdb.getCrystallographicInfo()==null || pdb.getCrystallographicInfo().getCrystalCell()==null) {
            LOGGER.warn("No crystal information found in source "+pdbCode+". Only asymmetric unit interfaces will be calculated.");
        }


        // for the webui
        modelAdaptor = new DataModelAdaptor();
        modelAdaptor.setParams(params, pdbCode);
        modelAdaptor.setPdbMetadata(pdb);

        if (!isInputAFile && params.getCifRepositoryTemplateUrl()!=null && params.isGenerateModelSerializedFile()) {
    ...
    private Structure readStructureFromPdbCode(FileParsingParameters fileParsingParams, String pdbCode) throws StructureException, IOException {
    ...
    public long doFindInterfaces() throws EppicException {
        ...
        System.out.println("Calculating possible interfaces for "+name+"...");
        ...
        checkClashes(isInputAFile);

        return System.currentTimeMillis() - start;
    }

    private void checkClashes(boolean isInputAFile) throws EppicException {
    ...
    public long doFindAssemblies() {
        ...
        System.out.println("Calculating possible assemblies for "+name+"...");
    ...
    public long doGeomScoring() throws EppicException {
        ...
        if (interfaces.size()==0) {
            ...
            System.out.println("No interfaces found for "+name+", nothing to analyse.");
    ...
    public long doWriteCoordFiles() throws EppicException {
        ...
        if (params.isGenerateThumbnails()) {
            System.out.println("Writing PyMOL files for " + name);
    ...
    public long doWriteAssemblyDiagrams() throws EppicException {
        ...
        System.out.println("Writing Assembly Diagram files for " + name);
...
//	}
    ...
    public long doCombinedScoring() throws EppicException {
        ...
        System.out.println("Done scoring for " + name);
