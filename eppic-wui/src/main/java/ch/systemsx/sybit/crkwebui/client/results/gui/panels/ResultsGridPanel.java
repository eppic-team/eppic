	...
	private void onClustersRadioValueChange(boolean value){
		if (value) { //check clusters event
			...
			if(ApplicationContext.getSelectedAssemblyId() > 0)
				History.newItem("clusters/" + ApplicationContext.getPdbInfo().getJobId() + "/" + ApplicationContext.getSelectedAssemblyId());
			else
				History.newItem("clusters/" + ApplicationContext.getPdbInfo().getJobId());
			...
			if(ApplicationContext.getSelectedAssemblyId() > 0)
				History.newItem("interfaces/" + ApplicationContext.getPdbInfo().getJobId() + "/" + ApplicationContext.getSelectedAssemblyId());
			else
				History.newItem("interfaces/" + ApplicationContext.getPdbInfo().getJobId());
