...
//this class is not needed at all! TODO remove
public class AssemblyClustersGridView extends GroupSummaryView<AssemblyItemModel> {

    public AssemblyClustersGridView(){
     ...
     */
    @Override
    public SafeHtml renderGroupHeader(GroupingData<AssemblyItemModel> groupInfo) {
	String component = "<div>";
	/*for(AssemblyItemModel m: groupInfo.getItems()){
     ...
     */
    @Override
    protected SafeHtml renderSummary(GroupingData<AssemblyItemModel> groupInfo, Map<ValueProvider<? super AssemblyItemModel, ?>, Number> data) {
	...
	for (int i = 0, len = colCount; i < len; i++) {  
	    SummaryColumnConfig<AssemblyItemModel,?> cf = (SummaryColumnConfig<AssemblyItemModel,?>) cm.getColumn(i);
