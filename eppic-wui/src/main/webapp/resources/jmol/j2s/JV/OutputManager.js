...
Clazz.load (null, "JV.OutputManager", ["java.lang.Boolean", "java.util.Date", "$.Hashtable", "$.Map", "JU.List", "$.PT", "$.Rdr", "$.SB", "J.api.Interface", "J.i18n.GT", "J.io.JmolBinary", "JW.Logger", "$.Txt", "JV.FileManager", "$.JC", "$.Viewer"], function () {
...
params.put ("logging", Boolean.$valueOf (JW.Logger.debugging));
...
var msg = (type.equals ("PDB") || type.equals ("PQR") ? this.vwr.getPdbAtomData (null, out) : type.startsWith ("PLOT") ? this.vwr.ms.getPdbData (modelIndex, type.substring (5), this.vwr.getSelectedAtoms (), parameters, out) : getCurrentFile ? out.append (this.vwr.getCurrentFileAsString ()).toString () : this.vwr.getFileAsBytes (pathName, out));
out.closeChannel ();
...
JW.Logger.error (this.vwr.setErrorMessage (sret = "ERROR creating image??: " + er, null));
} finally {
this.vwr.creatingImage = false;
...
}, "~S,~A,~N,java.util.Hashtable,~B,~S,~N,JU.List");
