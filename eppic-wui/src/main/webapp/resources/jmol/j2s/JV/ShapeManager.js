...
Clazz.load (["JU.BS"], "JV.ShapeManager", ["java.lang.Boolean", "JU.P3", "J.api.Interface", "J.c.PAL", "$.VDW", "JM.Atom", "JW.BSUtil", "JV.JC"], function () {
...
function (vwr) {
this.vwr = vwr;
this.gdata = vwr.getGraphicsData ();
}, "JV.Viewer");
...
shape.initializeShape (this.vwr, this.gdata, this.modelSet, shapeID);
this.vwr.setShapeErrorState (-1, null);
...
if (bsSelected == null && (shapeID != 1 || size != 2147483647)) bsSelected = this.vwr.getSelectedAtoms ();
if (rd != null && rd.value != 0 && rd.vdwType === J.c.VDW.TEMP) this.modelSet.getBfactor100Lo ();
...
function (bs) {
if (bs == null) bs = this.vwr.getSelectedAtoms ();
...
var atoms = this.modelSet.at;
var flag0 = -48;
for (var i = this.modelSet.ac; --i >= 0; ) {
var atom = atoms[i];
...
if (bsAtoms != null) {
var ptCenter = this.vwr.getAtomSetCenter (bsAtoms);
var pt =  new JU.P3 ();
this.vwr.transformPt3f (ptCenter, pt);
pt.add (ptOffset);
this.vwr.unTransformPoint (pt, pt);
pt.sub (ptCenter);
this.vwr.setAtomCoordsRelative (pt, bsAtoms);
ptOffset.set (0, 0, 0);
}this.modelSet.getRenderable (this.bsRenderableAtoms);
var vibrationVectors = this.modelSet.vibrations;
var atoms = this.modelSet.at;
var vibs = (vibrationVectors != null && this.vwr.isVibrationOn ());
for (var i = this.bsRenderableAtoms.nextSetBit (0); i >= 0; i = this.bsRenderableAtoms.nextSetBit (i + 1)) {
var atom = atoms[i];
var screen = (vibs && atom.hasVibration () ? this.vwr.transformPtVib (atom, vibrationVectors[i]) : this.vwr.transformPt (atom));
atom.sX = screen.x;
atom.sY = screen.y;
atom.sZ = screen.z;
var d = Math.abs (atom.madAtom);
if (d == JM.Atom.MAD_GLOBAL) d = Clazz.floatToInt (this.vwr.getFloat (1141899265) * 2000);
atom.sD = Clazz.floatToShort (this.vwr.scaleToScreen (screen.z, d));
}
if (this.vwr.getSlabEnabled ()) {
var slabByMolecule = this.vwr.getBoolean (603979940);
var slabByAtom = this.vwr.getBoolean (603979938);
...
for (; j < m.ac; j++, pt++) if (this.gdata.isClippedZ (atoms[pt].sZ - (atoms[pt].sD >> 1))) break;
...
}if (this.modelSet.getAtomCount () == 0 || !this.vwr.getShowNavigationPoint ()) return null;
var minX = 2147483647;
var maxX = -2147483648;
var minY = 2147483647;
var maxY = -2147483648;
...
function (modelSet) {
this.modelSet = this.vwr.ms = modelSet;
}, "JM.ModelSet");
...
JW.BSUtil.invertInPlace (bsSelected, this.vwr.getAtomCount ());
bsSelected.and (bsSubset);
}}JW.BSUtil.andNot (bsSelected, this.vwr.getDeletedAtoms ());
