...
this.ms.modelSetAuxiliaryInfo = info;
this.ms.modelSetProperties = this.ms.getModelSetAuxiliaryInfoValue ("properties");
...
}this.jmolData = this.ms.getModelSetAuxiliaryInfoValue ("jmolData");
this.fileHeader = this.ms.getModelSetAuxiliaryInfoValue ("fileHeader");
this.ms.trajectorySteps = this.ms.getModelSetAuxiliaryInfoValue ("trajectorySteps");
this.isTrajectory = (this.ms.trajectorySteps != null);
this.isPyMOLsession = this.ms.getModelSetAuxiliaryInfoBoolean ("isPyMOL");
this.doAddHydrogens = (this.jbr != null && !this.isTrajectory && !this.isPyMOLsession && !this.ms.getModelSetAuxiliaryInfoBoolean ("pdbNoHydrogens") && (this.ms.getModelSetAuxiliaryInfoBoolean ("pdbAddHydrogens") || this.vwr.getBooleanProperty ("pdbAddHydrogens")));
if (info != null) {
...
this.ms.someModelsHaveSymmetry = new Boolean (this.ms.someModelsHaveSymmetry | this.mergeModelSet.getModelSetAuxiliaryInfoBoolean ("someModelsHaveSymmetry")).valueOf ();
...
this.ms.someModelsHaveFractionalCoordinates = new Boolean (this.ms.someModelsHaveFractionalCoordinates | this.mergeModelSet.getModelSetAuxiliaryInfoBoolean ("someModelsHaveFractionalCoordinates")).valueOf ();
...
this.ms.modelSetAuxiliaryInfo.put ("someModelsHaveSymmetry", Boolean.$valueOf (this.ms.someModelsHaveSymmetry));
this.ms.modelSetAuxiliaryInfo.put ("someModelsHaveUnitcells", Boolean.$valueOf (this.someModelsHaveUnitcells));
this.ms.modelSetAuxiliaryInfo.put ("someModelsHaveFractionalCoordinates", Boolean.$valueOf (this.ms.someModelsHaveFractionalCoordinates));
this.ms.modelSetAuxiliaryInfo.put ("someModelsHaveAromaticBonds", Boolean.$valueOf (this.ms.someModelsHaveAromaticBonds));
}}, "~S,java.util.Map");
...
JW.Logger.info ("ModelSet: haveSymmetry:" + this.ms.someModelsHaveSymmetry + " haveUnitcells:" + this.someModelsHaveUnitcells + " haveFractionalCoord:" + this.ms.someModelsHaveFractionalCoordinates);
JW.Logger.info (this.adapterModelCount + " model" + (this.ms.mc == 1 ? "" : "s") + " in this collection. Use getProperty \"modelInfo\" or" + " getProperty \"auxiliaryInfo\" to inspect them.");
}var q = this.ms.getModelSetAuxiliaryInfoValue ("defaultOrientationQuaternion");
if (q != null) {
...
this.ms.setModelAuxiliaryInfo (this.baseModelIndex, "initialAtomCount", info.get ("initialAtomCount"));
this.ms.setModelAuxiliaryInfo (this.baseModelIndex, "initialBondCount", info.get ("initialBondCount"));
...
sb.append ("select *;");
var script = this.ms.getModelSetAuxiliaryInfoValue ("jmolscript");
...
var atomProperties = this.ms.getModelAuxiliaryInfoValue (i, "atomProperties");
...
JW.Logger.info ("creating " + key + " for model " + this.ms.getModelName (i));
...
}}if (this.ms.getModelAuxiliaryInfoValue (ipt, "periodicOriginXyz") != null) this.ms.someModelsHaveSymmetry = true;
}
var m = this.ms.am[this.baseModelIndex];
this.vwr.setSmilesString (this.ms.modelSetAuxiliaryInfo.get ("smilesString"));
var loadState = this.ms.modelSetAuxiliaryInfo.remove ("loadState");
var loadScript = this.ms.modelSetAuxiliaryInfo.remove ("loadScript");
...
this.ms.setModelAuxiliaryInfo (this.baseModelCount, "trajectoryStepCount", Integer.$valueOf (n));
...
if (atomInfo != null) this.ms.setModelAuxiliaryInfo (modelIndex, "PDB_CONECT_firstAtom_count_max", atomInfo);
}var models = this.ms.am;
var atoms = this.ms.at;
models[modelIndex].bsAtoms.set (atoms.length + 1);
models[modelIndex].bsAtoms.clear (atoms.length + 1);
var codes = this.ms.getModelAuxiliaryInfoValue (modelIndex, "altLocs");
...
this.ms.setModelAuxiliaryInfo (modelIndex, "altLocs", codes);
}codes = this.ms.getModelAuxiliaryInfoValue (modelIndex, "insertionCodes");
models[modelIndex].setNInsertions (codes == null ? 0 : codes.length);
var isModelKit = (this.ms.modelSetName != null && this.ms.modelSetName.startsWith ("Jmol Model Kit") || modelName.startsWith ("Jmol Model Kit") || "Jme".equals (this.ms.getModelAuxiliaryInfoValue (modelIndex, "fileType")));
...
this.ms.setModelAuxiliaryInfo (i, "fileType", this.ms.modelSetTypeName);
if (this.fileHeader != null) this.ms.setModelAuxiliaryInfo (i, "fileHeader", this.fileHeader);
...
this.ms.setModelAuxiliaryInfo (i, "modelName", modelNames[i]);
this.ms.setModelAuxiliaryInfo (i, "modelNumber", Integer.$valueOf (modelNumbers[i] % 1000000));
this.ms.setModelAuxiliaryInfo (i, "modelFileNumber", Integer.$valueOf (this.ms.modelFileNumbers[i]));
this.ms.setModelAuxiliaryInfo (i, "modelNumberDotted", this.ms.getModelNumberDotted (i));
var codes = this.ms.getModelAuxiliaryInfoValue (i, "altLocs");
if (codes != null) {
JW.Logger.info ("model " + this.ms.getModelNumberDotted (i) + " alternative locations: " + codes);
}}
}, "~N");
...
}var atom = this.ms.addAtom (this.currentModelIndex, this.nullGroup, atomicAndIsotopeNumber, atomName, atomSerial, atomSite, xyz, radius, vib, formalCharge, partialCharge, occupancy, bfactor, tensors, isHetero, specialAtomID, atomSymmetry);
...
if (haveMultipleBonds && this.ms.someModelsHaveSymmetry && !this.vwr.getBoolean (603979794)) JW.Logger.info ("ModelSet: use \"set appletSymmetryToBonds TRUE \" to apply the file-based multiple bonds to symmetry-generated atoms.");
...
this.ms.unitCells[i].setSymmetryInfo (i, this.ms.getModelAuxiliaryInfo (i));
}}
...
i0 = this.baseAtomIndex + this.ms.getModelAuxiliaryInfoInt (iModel, "presymmetryAtomIndex") + this.ms.getModelAuxiliaryInfoInt (iModel, "presymmetryAtomCount");
...
Clazz.defineMethod (c$, "initializeBonding", 
 function () {
var bsExclude = (this.ms.getModelSetAuxiliaryInfoValue ("someModelsHaveCONECT") == null ? null :  new JU.BS ());
...
var modelBondCount = this.ms.getModelAuxiliaryInfoInt (i, "initialBondCount");
var modelIsPDB = models[i].isBioModel;
if (modelBondCount < 0) {
modelBondCount = this.ms.bondCount;
}var modelHasSymmetry = this.ms.getModelAuxiliaryInfoBoolean (i, "hasSymmetry");
var doBond = (forceAutoBond || doAutoBond && (modelBondCount == 0 || modelIsPDB && this.jmolData == null && (this.ms.getModelSetAuxiliaryInfoBoolean ("havePDBHeaderName") || modelBondCount < Clazz.doubleToInt (modelAtomCount / 2)) || modelHasSymmetry && !symmetryAlreadyAppliedToBonds && !this.ms.getModelAuxiliaryInfoBoolean (i, "hasBonds")));
if (!doBond) continue;
autoBonding = true;
...
if (this.group3Lists != null) if (this.ms.modelSetAuxiliaryInfo != null) {
...
var bs = JW.JmolMolecule.getBranchBitSet (this.ms.at, b.atom2.i, bsToTest, null, b.atom1.i, false, true);
bs.set (b.atom2.i);
...
this.ms.modelSetAuxiliaryInfo.put ("bsDeletedAtoms", bsDeletedAtoms);
...
var i = -1;
var n = 0;
var loadAllData = (JW.BSUtil.cardinalityOf (bsSelected) == vwr.getAtomCount ());
